...
thread_local boost::random::mt19937 rng_arc{ std::random_device{}() };
static Botan::System_RNG rng;
using namespace drogon;
using namespace drassist;

inline uint32_t arc4random_uniform_cross(uint32_t upper_bound)
{
	if (upper_bound < 2) {
		return 0;
	}
	boost::random::uniform_int_distribution<uint32_t> dist(0, upper_bound - 1);
	return dist(rng_arc);
}

uint32_t drassist::secureRandom(uint32_t lower, uint32_t upper)
{
	return lower + arc4random_uniform_cross(upper - lower);
}
